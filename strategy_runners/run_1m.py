...
log_forward_test(
    ...
    pattern="force-init",
    decision="SKIPPED",
    reason="force sheet create",
    candle_time=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    candle_price=0,
    confidence_score=0,
    sheet_name="1m Forward Test"
)
smart_ai = SmartAIV2(sl=10, rr=6, min_confidence=50)
last_candle_time = None

print("[SmartAI 1m] Forward testing started on ETHUSDT 1-minute candles...")
print(f"📥 Incoming log_forward_test called with sheet_name = 1M")
while True:
    ...
    if candle and candle["timestamp"] != last_candle_time:
        ...
        if signal:
            print(f"[1m SIGNAL] {signal['type']} @ {signal['entry']} | SL: {signal['sl']} | TP: {signal['tp']} | Confidence: {signal['confidence_score']}")
            ...
            if signal["confidence_score"] >= smart_ai.min_confidence:
                ...
                add_open_trade(signal, timeframe="1m")
            else:
                decision = "SKIPPED"
            ...
            print("[SmartAI 1m] No signal on this candle.")
